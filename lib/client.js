...
var Client = {
	...
	onPeering : function(receivedPeeringPacket, callingPeer) {
		...
		if((client.pool.usedConnections < client.pool.size) && !client.pool.exists(receivedPeeringPacket.body.originator)) {
			...
			var offer = {};
			offer.type = receivedPeeringPacket.body.offer.type;
			offer.sdp = null;
			
			console.log(receivedPeeringPacket.body.offer.sdp);

			var pkt = new p2pPacket('peering', 
					new peeringPacket(receivedPeeringPacket.body.offer, receivedPeeringPacket.body.candidates, receivedPeeringPacket.body.originator, null));
