...
var Client = {
	...
	console.log('Peering req received');
		...
		if((this.pool.usedConnections < this.pool.size) && !this.pool.exists(receivedPeeringPacket.body.originator)) {
			...
			console.log(this.pool[selectedPeerId]);
			...
			if(peersIdArray.length > 0) {
				...
				var pacchettoBody = new peeringPacket(receivedPeeringPacket.body.offer, receivedPeeringPacket.body.candidates, receivedPeeringPacket.body.port);
				pacchettoBody.port = receivedPeeringPacket.port;
				var stoCazzoDiPacchetto = new p2pPacket('peering', JSON.stringify(pacchettoBody), false);
