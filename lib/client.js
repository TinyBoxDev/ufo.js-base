...
var Client = {
	...
	densify : function() {
		if(this.pool.usedConnections < 3) {
			...
			//newPeer.
			this.pool[this.pool[selected]].lookForAPeer(function(socket, retrievedPeerId) {
				var newPeer = new Peer();
				client.pool.pushConnection(retrievedPeerId, newPeer);
				if(socket)
					newPeer.channel.connectViaSocket(socket, retrievedPeerId);
