...
var Client = {
	...
	onPeering : function(receivedPeeringPacket, callingPeer) {
		...
		var self = this;
		if((self.pool.usedConnections < this.pool.size) && !self.pool.exists(receivedPeeringPacket.body.originator)) {
			...
			//	this.pool[selectedPeerId].send(receivedPeeringPacket);
				self.pool[selectedPeerId].send(new p2pPacket('echo', receivedPeeringPacket, true));
			} else {
