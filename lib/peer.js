...
Peer.prototype.createSocketForPeer = function(peeringPacket, callingPeer, onConnected) {
	...
	peeringPacket.body.offer += '\r\n';
	this.peerConnection.setRemoteDescription(new RTCSessionDescription({ 'type' : 'offer', 'sdp' : peeringPacket.body.offer }), onRemoteDescriptionAdded, generalFailureCallback);
	...
	this.peerConnection.onicecandidate = function(event) {
		if (event.candidate) {
			...
			if (candidatesArray.length == 4) {
				...
				//var offer = JSON.stringify(self.peerConnection.localDescription);
				...
				var offer = self.peerConnection.localDescription.sdp;
				offer = offer.substring(offer.length-2, 2);
				self.channel.send(new p2pPacket('peering', 
							new peeringPacket(offer, candidatesArray, client.id, self.localPort), true));
			}
		}
	}
