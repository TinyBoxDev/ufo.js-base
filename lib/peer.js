...
Peer.prototype.createSocketForPeer = function(peeringPacket, callingPeer, onConnected) {
	...
	//this.peerConnection.setRemoteDescription(new RTCSessionDescription(JSON.parse(offer)), onRemoteDescriptionAdded, generalFailureCallback);
        ...
        this.peerConnection.setRemoteDescription(new RTCSessionDescription({ 'type' : 'offer', 'sdp' : unescape(peeringPacket.body.offer) }), onRemoteDescriptionAdded, generalFailureCallback);
	...
	this.peerConnection.onicecandidate = function(event) {
		if (event.candidate) {
			...
			if (candidatesArray.length == 4) {
				...
				var offer = escape(self.peerConnection.localDescription.sdp);
				self.channel.send(new p2pPacket('peering', 
