...
Peer.prototype.createSocketForPeer = function(peeringPacket, callingPeer, onConnected) {
	...
	this.peerConnection.setRemoteDescription(peeringPacket.toOffer(), onRemoteDescriptionAdded, generalFailureCallback);
	...
	this.peerConnection.onicecandidate = function(event) {
		if (event.candidate) {
			...
			if (candidatesArray.length == 4) {
				...
				var peeringPacket = new peeringPacket(client.id);
				self.channel.send(new p2pPacket('peering', peeringPacket.fromOffer(self.peerConnection.localDescription), true));
			}
		}
	}
